...
const BannerRealEstate = () => {
  ...
  useEffect(() => {
    fetch(`http://localhost:5000/banner-realestate`)
      .then((res) => res.json())
        ...
        <div className="auto-container">
          <div className="inner-box">
            <div className="content-box mb_80">
              <h2>{b.bannerHeading} <span>{b.bannerTextTwo}</span>  <br />{b.bannerText}</h2>
              <p>{b.bannerDetails}</p>
              <div className="btn-box">
                <a href={b.buttonURL} className="jos button relative z-[1] inline-flex items-center gap-3 rounded-[50px] border-none bg-colorViolet py-[8px] text-white after:bg-colorOrangyRed hover:text-white mr_20">{b.buttonText}</a>
                <a href={b.buttonURLTwo} className="theme-btn btn-two">{b.buttonTextTwo}</a>
              </div>
            </div>
            <figure className="image-box"><img src={b.imageOne} alt /></figure>
          </div>
        </div>
      </section>)
    }
